...
const GlobalStats = ({ classes }: GlobalStatsProps) => {
  const stats = useMemo(() => {
    ...
    let bestClass = { name: "N/A", average: "N/A" };
    ...
    let worstClass = { name: "N/A", average: "N/A" };
    if (classAverages.length > 0) {
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Based on all entered marks</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground truncate">{stats.bestClass.name}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Lowest Performing Class</CardTitle>
            <TrendingDown className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.worstClass.average}</div>
            <p className="text-xs text-muted-foreground truncate">{stats.worstClass.name}</p>
          </CardContent>
        </Card>
      </div>
    </div>
