...
export const ClassReportsTab = ({
  ...
  const isProcessing = actionState.status === "loading" || actionState.status === "saving";
      ...
      <div className="space-y-6">
        ...
        <Card className="w-full">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            <Button variant="outline" disabled={isProcessing} onClick={() => runAction(() => onExportPdf(exportLanguage))} className="justify-start h-11 text-xs sm:text-sm">
              <FileText className="mr-3 h-4 w-4 text-blue-500 shrink-0" /> Class Marksheet (PDF)
            </Button>
            <Button variant="outline" disabled={isProcessing} onClick={() => runAction(() => onExportCsv(exportLanguage))} className="justify-start h-11 text-xs sm:text-sm">
              <FileText className="mr-3 h-4 w-4 text-slate-500 shrink-0" /> Export Data (CSV)
            </Button>
            <Button variant="outline" disabled={isProcessing} onClick={() => runAction(() => onExportBulkPdf(exportLanguage))} className="justify-start h-11 text-xs sm:text-sm">
              <Download className="mr-3 h-4 w-4 text-green-500 shrink-0" /> Learner Report Cards
            </Button>
            <Button variant="outline" disabled={isProcessing} onClick={() => runAction(() => onExportBlankList(exportLanguage))} className="justify-start h-11 text-xs sm:text-sm">
              <FileText className="mr-3 h-4 w-4 text-orange-500 shrink-0" /> Blank List (PDF)
            </Button>
            <Button variant="outline" disabled={isProcessing} onClick={() => runAction(() => onShare(exportLanguage))} className="justify-start h-11 text-xs sm:text-sm">
              <Share2 className="mr-3 h-4 w-4 text-indigo-500 shrink-0" /> Class Summary
            </Button>
            <Button variant="outline" disabled={isProcessing} onClick={() => runAction(onOpenDiagnostic)} className="justify-start h-11 text-xs sm:text-sm">
              <BarChart3 className="mr-3 h-4 w-4 text-primary shrink-0" /> Diagnostic Report
            </Button>
            <Button variant="outline" onClick={() => runAction(onSasams)} disabled={!isLocked || isProcessing} className="justify-start h-11 text-xs sm:text-sm">
