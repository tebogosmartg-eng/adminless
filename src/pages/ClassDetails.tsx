...
import { prepareExportData } from "@/utils/exportData";
import { exportToExcel, exportToWord } from "@/utils/exportFormatters";
...
const ClassDetailsContent = () => {
  ...
  const getExportData = () => {
      if (!classInfo || !activeTerm) return null;
      ...
      return prepareExportData(classInfo, termAssessments, termMarks, gradingScheme, schoolName, teacherName, activeTerm.name);
  };
  ...
  const handleSASAMSExportAction = () => {
      ...
      if (hasUnsavedChanges) {
          showError("Export Blocked: You have unsaved changes.");
          return;
      }
  ...
  if (classesLoading || academicLoading) {
    return <div className="flex h-[50vh] w-full items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary opacity-30" /></div>;
  }

  if (!classId || !classInfo) return <div className="p-8 text-center text-muted-foreground">Class not found.</div>;
    ...
    <div className="container mx-auto p-2 sm:p-4 w-full max-w-7xl space-y-6 pb-20 relative animate-in fade-in duration-700">
      ...
      <Tabs defaultValue="assessments" className="w-full max-w-full">
        <TabsList className="flex items-center justify-start w-full h-auto min-h-[48px] bg-muted/50 border p-1 overflow-x-auto no-scrollbar gap-1 rounded-xl flex-nowrap">
          ...
          <TabsTrigger value="capture" className="flex-none shrink-0 h-10 px-4 sm:px-6 gap-2 rounded-lg font-bold text-xs sm:text-sm"><Camera className="h-3.5 w-3.5" /> Capture</TabsTrigger>
          <TabsTrigger value="reports" className="flex-none shrink-0 h-10 px-4 sm:px-6 gap-2 rounded-lg font-bold text-xs sm:text-sm">Reports</TabsTrigger>
        </TabsList>
        
        <TabsContent value="assessments" className="mt-4">
            <MarkSheet classInfo={classInfo} onViewLearnerProfile={(l) => dialogs.setSelectedProfileLearner(l)} />
        </TabsContent>
        ...
        <TabsContent value="reports" className="mt-4 space-y-8">
             <ClassReportsTab 
               ...
               onExportExcel={() => { const d = getExportData(); if(d) exportToExcel(d); }}
               onExportWord={() => { const d = getExportData(); if(d) exportToWord(d); }}
             />
        </TabsContent>
      </Tabs>
...
const ClassDetails = () => {
  ...
  if (!authReady || !user) return <div className="flex h-[50vh] w-full items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary opacity-50" /></div>;
